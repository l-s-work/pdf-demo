...
from app.models import PdfHighlightHit, PdfPageMeta
...
# 处理上传任务，完成原始文件、线性化预览 PDF 与命中记录入库。
async def process_ingest_job(session: AsyncSession, job_id: str) -> dict[str, object]:
    ...
    derived_version = int(request_payload_dict.get('derivedVersion', request_payload_dict.get('previewVersion', DERIVED_VERSION)))
    ...
    derived_object_key = str(
        ...
        or build_derived_pdf_object_key(job.pdf_id, derived_version)
    )
    ...
    with TemporaryDirectory(prefix=f'pdf_{job.pdf_id}_') as temp_dir:
        ...
        try:
            ...
            total_hits = 0
            page_words_cache: dict[int, list[dict[str, float | int | str]]] = {}

            for item in request_items:
                keyword = str(item.get('keyword', '')).strip()
                requested_page_num = int(item.get('pageNum', 1))
                start_page_num = min(max(1, requested_page_num), total_pages)
                rect_segments = locate_keyword_near_page(pdf_doc, keyword, start_page_num, page_words_cache)
                if not rect_segments:
                    # DOCX 经过版式重排后，原始页码往往会偏移很大，兜底全局搜一次。
                    rect_segments = locate_keyword_anywhere(pdf_doc, keyword, page_words_cache)
